...
if(isset($_POST['update_profile'])){
   ...
   mysqli_query($cn, "UPDATE `users` SET U_Name = '$uname', U_Email='$email', U_Image = '$image' WHERE User_Id = '$reader_id'") or die('query failed');
   ...
   if(!empty($update_pw) AND !empty($new_pw) AND !empty($c_pw)){
      if($update_pw != $old_pw){
         ...
         mysqli_query($cn, "UPDATE `users` SET password='$new_pw' WHERE User_Id = '$reader_id'") or die('query failed');
...
<h1 class="title">update <span>profile</span></h1>
   ...
   <?php
         $res = mysqli_query($cn, "SELECT * FROM `users` WHERE User_Id='$reader_id'") or die('query failed');
